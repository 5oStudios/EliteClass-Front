...
const InPersonSessionOverview = () => {
  ...
  useEffect(() => {
    if (slug) {
      axios
        .get(`session/detail/${slug}?secret=11f24438-b63a-4de2-ae92-e1a1048706f5&&type=${type}`)
        .then((response) => {
  ...
  const onEnroll = async (isFreeEnroll = false) => {
    ...
    if (isFreeEnroll) {
      ...
      const obj = {
        offline_session_id: sessiondetail?.id,
      };
      config = {
        method: 'post',
  ...
  const meetingToCart = async () => {
    ...
    const config = {
      method: 'post',
      url: is_cart ? 'remove/cart' : 'addtocart/offline-session',
      data: obj,
    };
    const res = await axios(config)
      .then(async (response: any) => {
        ...
        showNotification({
          message: is_cart
            ? 'In-Person Session removed from cart successfuly!'
            : 'In-Person Session added to cart successfuly!',
          color: 'green',
        });
  ...
  function renderEnrollButton(isFreeEnroll = false) {
    let aa = parseInt(sessiondetail?.discount_price);
    //debugger;
    return (
      <Stack
        id="btn-inPersonEnroll"
  ...
  return (
    <>
      ...
      <Seo title="session overview" description="Best LMS" path={`in-person-sessions/${slug}`} />
      <>
...
export default InPersonSessionOverview;
