...
const InPersonSessions = ({
  ...
  client.invalidateQueries([`sessions`, cost, rating, search]);
  ...
  const getData = async ({ pageParam = 1 }) => {
    ...
    const dataFromServer = await axios.post(`in-person/sessions?page=${pageParam}`, obj);
  ...
  return (
    <>
      ...
      <Seo title="In-Person-sessions" description="Best Lms" path="/in-person-sessions" />
      <Box component="main" mb={30}>
        <Container p={0}>
          <PageHeader
            ...
            rightSection={
              <Group mr={10}>
                <ActionIcon id="btn-inpersonLayout" variant="transparent" onClick={() => setLayoutGrid(!layoutGrid)}>
                  {layoutGrid ? (
                    colorScheme == 'dark' ? (
                      <GridLayoutIconWhite />
                    ) : (
                ...
                >
                  {data?.pages.map((packages: any, i) =>
                    packages.results.map((course: any) => {
                      return (
                        <LiveSessionCard
                          href={`/in-person-sessions/${course?.id}`}
                          // href={{
                          //   pathname: `/in-person-sessions/${course?.id}`,
                          //   query: {
                          //     id: course?.id,
                          //     //type: course?.type,
                          //   },
