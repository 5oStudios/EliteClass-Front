...
const SeeAllPage: NextPage = () => {
  // language
  const router = useRouter();
  const t = router.locale === 'ar-kw' ? ar : en;
  // language
  const { slug, isBundled = true } = router.query;
  ...
  const getCourseDetails = async (slug: string, isBundled: string = 'false') => {
    try {
      const response: any = await axios.post('course/detail', {
        course_id: slug,
        is_bundle: JSON.parse(isBundled),
  ...
  const { data, isLoading, isError, error, isRefetching, refetch } = useQuery<any, any>(
    ...
    () => getCourseDetails(slug as string, isBundled as string)
  );
