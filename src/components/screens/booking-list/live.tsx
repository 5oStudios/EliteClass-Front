...
export const Live = (props: any) => {
  ...
  const getLiveData = async ({ pageParam = 1 }) => {
    const dataFromServer = await axios.get(`streaming/booking/list?page=${pageParam}`);
  ...
  const { data, isLoading, isFetching, isFetchingNextPage, isError, fetchNextPage, hasNextPage } =
    useInfiniteQuery(['streaming/booking/list', 'live'], getLiveData, {
