...
export const InPerson = (props: any) => {
  ...
  const getInpersonData = async ({ pageParam = 1 }) => {
    const dataFromServer = await axios.get(`wishlist/sessions?page=${pageParam}`);
  ...
  //   useInfiniteQuery(['wishlist/sessions', 'in-person'], getInpersonData, {
  ...
  const { data, isLoading, refetch, isFetchingNextPage, fetchNextPage, hasNextPage } =
    useInfiniteQuery(
      ...
      ({ pageParam }) =>
        getPaginatedData({
          page: pageParam,
          endpoint: 'wishlist/sessions?secret=11f24438-b63a-4de2-ae92-e1a1048706f5',
        }),
      {
        getNextPageParam: (lastPage) => lastPage.next,
      }
            ...
            {data?.pages?.map((page: any) =>
              page.results.map((item: any) => (
                <LiveSessionCard
                  ...
                  //@ts-ignore
                  href={{
                    pathname: `/in-person-sessions/${item?.meeting_id}`,
                    query: {
                      id: item?.meeting_id,
                      type: 'in-person',
                    },
